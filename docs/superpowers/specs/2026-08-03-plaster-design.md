# Plastr — Interactive Assembly Graph Studio
...
4. "Easy to use" means: `conda env create -f environment.yml`, then one command
   (`plastr view graph.gfa`) that opens a browser. No build step, no Qt, no X11 quirks.

## Architecture

Three layers, each independently testable.

**Core library (`plastr.core`)** — pure Python, no I/O beyond files, no web knowledge.
...
**Service layer (`plastr.server`)** — a FastAPI app that holds one `Project` in memory and
exposes it over REST/JSON. It owns session state, undo/redo, and file export. It contains no
biology.

**Presentation (`plastr.web`)** — static HTML/CSS/JS, no build step. A canvas renderer, a
...
Parsers raise a typed `PlastrFormatError` carrying file, line number, and the offending text;
