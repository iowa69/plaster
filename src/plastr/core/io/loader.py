...
def read_contigs_fasta(path: str | os.PathLike[str]) -> AssemblyGraph:
    ...
    if not graph.segments:
        raise PlastrFormatError("FASTA contained no sequences", str(path))
    return graph
...
def load_graph(path: str | os.PathLike[str], fmt: str | None = None) -> AssemblyGraph:
    ...
    if not os.path.exists(path):
        raise PlastrFormatError(f"file not found: {path}")
    if os.path.isdir(path):
        raise PlastrFormatError(f"{path} is a directory, not an assembly file")
    if not os.access(path, os.R_OK):
        raise PlastrFormatError(f"cannot read {path}: permission denied")
...
def load_spades_paths(graph: AssemblyGraph, path: str | os.PathLike[str]) -> int:
    ...
    """
    if not os.path.exists(path):
        raise PlastrFormatError(f"paths file not found: {path}")
    added = 0
