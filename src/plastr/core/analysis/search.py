...
from ..errors import PlastrFormatError, MissingDependencyError
...
def parse_query(query: str) -> list[tuple[str, str]]:
    ...
    text = (query or "").strip()
    if not text:
        raise PlastrFormatError("empty search query")
...
def _blast(
    ...
    with tempfile.TemporaryDirectory(prefix="plastr_blast_") as tmp:
...
def _minimap(
    ...
    with tempfile.TemporaryDirectory(prefix="plastr_search_") as tmp:
