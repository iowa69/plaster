...
from .errors import AssemblageError, GraphOperationError
...
class Project:
    ...
    def require_graph(self) -> AssemblyGraph:
        if self.graph is None:
            raise AssemblageError("no assembly has been loaded yet")
        return self.graph
    ...
    def export(self, outdir: str, what: list[str] | None = None) -> list[dict]:
        ...
        if ("scaffolds" in what or "agp" in what) and self.plan is not None:
            ...
            write_scaffolds(
                ...
                agp_path,
                comments=[
                    f"generated by Assemblage from {self.source_path}",
