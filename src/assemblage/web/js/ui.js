...
export function createApp() {
  ...
  app.runExport = () => guard('Writing files…', async () => {
    ...
    const outdir = $('export-outdir').value.trim() || 'assemblage_out';
  ...
  app.savePNG = async () => {
    try {
      ...
      download(blob, 'assemblage-graph.png');
  ...
  app.saveSVG = () => {
    try {
      download(renderer.toSVG(), 'assemblage-graph.svg', 'image/svg+xml');
  ...
  async function browseTo(path) {
    try {
      ...
      list.innerHTML = data.entries.map((e) =>
        ...
        || '<li class="muted">nothing here that Assemblage can read</li>';
...
export async function start() {
  try {
    const saved = localStorage.getItem('assemblage-theme');
  ...
  window.assemblage = app; // handy for debugging from the console
  return app;
}
